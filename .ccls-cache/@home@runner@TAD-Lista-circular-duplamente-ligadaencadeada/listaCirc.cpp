...
bool LinkedList::insert(int val, std::string nome ){
	NodeLL *aux;
    if (!isFull()){
      aux = new NodeLL(val, nome, nullptr);
      if (isEmpty()){ 
        head = aux;
      }else { 
      	aux->setProx(head);
...
bool LinkedList::append(int val, std::string nome){
	NodeLL *aux, *pAnda;
    if (!isFull()){
      aux = new NodeLL(val, nome, nullptr);
      if (isEmpty()){ 
...
pAnda = head;
while (pAnda != nullptr) {
      std::cout << "["<< pAnda->getVal() << "]" << pAnda->getNome() << "\n";
      pAnda = pAnda->getProx();
    }
